...
cX_bridge_prev = cY_bridge_prev = xb_past_prev = yb_past_prev = xb_b4_prev = yb_b4_prev = 0.0
...
def getBridgeCenter(frame):
    ...
    corners = cv2.goodFeaturesToTrack(frame,4,0.01,10) #pick out 4 most prominant corners
    #draw circles
...
def lowPassWaypoints(cX_bridge,cY_bridge,xb_past,yb_past,xb_b4,yb_b4,cX_bridge_prev,cY_bridge_prev,xb_past_prev,yb_past_prev,xb_b4_prev,yb_b4_prev):
    ...
    cX_bridge = B*cX_bridge + (1-B)*cX_bridge_prev
    cY_bridge = B*cY_bridge + (1-B)*cY_bridge_prev
    xb_past = B*xb_past + (1-B)*xb_past_prev
    yb_past = B*yb_past + (1-B)*yb_past_prev
    xb_b4 = B*xb_b4 + (1-B)*xb_b4_prev
    yb_b4 = B*yb_b4 + (1-B)*yb_b4_prev
    return cX_bridge,cY_bridge,xb_past,yb_past,xb_b4,yb_b4
...
def main():
	...
	while not rospy.is_shutdown():
		...
		try: 
			...
			_,contours,_ = cv2.findContours(frame, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
			frame = np.zeros(shape=[height,width, 1], dtype=np.uint8)
			max_contour_ratio = .01  #change based on relative size of bridge in frame 
			...
			if len(contours) > 0:
			    ...
			    if areas[idxs[-1]] > max_contour_ratio*height*width: 
			        ...
			        # print('')
			        # print('Distance to first waypoint in body frame:')
			        # print(round(xb_b4,3),round(yb_b4,3))
			        # print('Distance to second waypoint in body frame:')
			        # print(round(xb_past,3),round(yb_past,3))
			        # print('Distance to center in body frame:')
			        # print(round(cX_bridge,3),round(cY_bridge,3))
			        # print('theta:')
			        # print(180.0/3.14*theta,' degrees')
			...
			# filledBridge = bridge.cv2_to_imgmsg(filledBridge, "8UC1")
			# corners = bridge.cv2_to_imgmsg(np.asarray(corners), "8UC1")
			# orig_frame = bridge.cv2_to_imgmsg(orig_frame, "8UC1")
		...
		# pub_filledBridge_image.publish(filledBridge)
		# pub_corners_image.publish(corners)
		# pub_orig_frame_image.publish(orig_frame)
		rate.sleep()
